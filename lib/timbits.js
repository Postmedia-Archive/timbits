...
function loadHelpers() {
  /*
  var folder = path.join(config.home, "helpers");
  
  filteredFiles(folder, /\.(coffee|js)$/).forEach(function(file) {
    var name = file.substring(0, file.lastIndexOf("."));
    timbits.log.notice("Loading handlebar helper: " + name);
    hbs.registerHelper(name, require(path.join(folder, file)));
  });
  */
}
...
function loadViews(timbit) {
  ...
  filteredFiles(folder, /\.html/).forEach(function(file) {
...
this.log = new Log(process.env.TIMBITS_VERBOSITY || Log.NOTICE);
...
this.serve = function(options) {
  ...
  if (config.engine.callback)
    app.engine(config.engine.extension, config.engine.callback);
  ...
  */
  ...
  loadTimbits(function() {
    try {
       ...
       timbits.log.notice("Timbits server listening on port " + timbits.server.address().port + " in " + app.settings.env + " mode");
    } catch (err) {
...
this.add = function(name, timbit, callback) {  
  timbits.log.notice("Placing " + name + " in the box");
  ...
  timbits.app.get(config.base + "/" + name + "/test/:which?", function(req, res) {
    ...
    if (config.test) {
      ...
      timbit.test("http://" + req.headers.host, alltests, function(results) {
        res.send(ck.render(views.timbit_test, {
          name: timbit.name,
          results: results
        }));
      });
